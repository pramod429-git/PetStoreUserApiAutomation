...
//this keyword not required since we are mentioning as current user for standerd practice we have used
		Response response=UserEndPoints.getUser(this.data.getUsername());
		response.then().log().all();
	...
	void testUpdateUser() {
		...
		Response response=UserEndPoints.updateUser(data, this.data.getUsername());
	...
	void testDeleteUser() {
		...
		Response response=UserEndPoints.deleteUser(this.data.getUsername());
